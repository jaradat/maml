...
class MAML:
    ...
    def construct_model(self, input_tensors=None, prefix='metatrain_'):
        ...
        if 'train' in prefix:
            ...
            self.metaval_total_loss1 = total_loss1 = tf.reduce_sum(lossesa) / tf.compat.v1.to_float(FLAGS.meta_batch_size)

            self.metaval_total_losses2 = total_losses2 = [tf.reduce_sum(lossesb[j]) / tf.compat.v1.to_float(FLAGS.meta_batch_size) for j in range(num_updates)]
    ...
    def construct_fc_weights(self):
        ...
        weights['w1'] = tf.compat.v1.Variable(tf.random.truncated_normal([self.dim_input, self.dim_hidden[0]], stddev=0.01))

        weights['b1'] = tf.compat.v1.Variable(tf.zeros([self.dim_hidden[0]]))

        for i in range(1,len(self.dim_hidden)):

            weights['w'+str(i+1)] = tf.compat.v1.Variable(tf.random.truncated_normal([self.dim_hidden[i-1], self.dim_hidden[i]], stddev=0.01))

            weights['b'+str(i+1)] = tf.compat.v1.Variable(tf.zeros([self.dim_hidden[i]]))

        weights['w'+str(len(self.dim_hidden)+1)] = tf.compat.v1.Variable(tf.random.truncated_normal([self.dim_hidden[-1], self.dim_output], stddev=0.01))
